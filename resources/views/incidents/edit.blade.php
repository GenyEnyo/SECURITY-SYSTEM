...
<head>
  ...
  <title>Edit Incident · M Dashboard</title>
...
<body data-page="incidents" data-crumbs='[{"label":"Incidents","href":"/incidents"},{"label":"Edit #{{ $incident->id }}"}]'>
            ...
            <div class="col-lg-6">
              ...
              <select id="f-status" name="incident_status_id" class="brand-input brand-select">
                @foreach ($statuses as $status)
                  <option value="{{ $status->id }}" @selected(old('incident_status_id', $incident->incident_status_id) == $status->id)>{{ $status->name }}</option>
                @endforeach
              </select>
