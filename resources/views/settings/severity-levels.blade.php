...
@section('page', 'severity-levels')
@section('crumbs', '[{"label":"Settings"},{"label":"Severity Levels"}]')
...
@section('content')
  <div class="page-head">
    <div>
      <h1 class="page-title">Settings — Severity Levels</h1>
      <p class="page-subtitle">Manage the severity / priority levels available on incidents.</p>
    </div>
    <div class="actions">
  ...
  <table id="severities-table" class="brand-table">
    <thead>
      <tr>
    ...
    <tbody>
      @foreach ($severities as $severity)
        <tr>
          <td class="fw-7">{{ $severity->id }}</td>
          <td><span class="pill" style="background:{{ $severity->color }}1f;color:{{ $severity->color }};">{{ $severity->name }}</span></td>
          <td>
      ...
      <div class="modal-content" style="border-radius:14px;border:0;">
        <form action="{{ route('settings.severity-levels.store') }}" method="POST">
          ...
          <div class="modal-header">
            <h5 class="modal-title fw-7">Add severity level</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
          </div>
          <div class="modal-body">
            <label class="field-label">Name</label>
            <input name="name" class="brand-input" placeholder="e.g. Critical" required autofocus style="height:54px;">

            <label class="field-label mt-3">Badge color</label>
            <input type="color" name="color" value="#6b7280" required
                   style="height:42px;width:80px;padding:4px;border:1px solid var(--border-soft);border-radius:8px;background:#fff;cursor:pointer;">
          </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content" style="border-radius:14px;border:0;">
        <form method="POST">
          ...
          @method('PUT')
          <div class="modal-header">
            <h5 class="modal-title fw-7">Edit severity level</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
          </div>
          <div class="modal-body">
            <label class="field-label">Name</label>
      ...
      <div class="modal-content" style="border-radius:14px;border:0;">
        <form method="POST">
          ...
          <div class="modal-body p-4 text-center">
            ...
            <h5 class="fw-7 mb-2">Delete this severity level?</h5>
            <p class="muted fw-5 mb-4">"<span class="target-name fw-7"></span>" will be removed permanently. Existing incidents that reference it cannot be deleted.</p>
